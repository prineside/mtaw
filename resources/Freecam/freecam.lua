...
function _getKeyState(key)
	return not ( keyStateCache[ key ] == nil or keyStateCache[ key ] == false )
end

bindKey( "lshift", "both", function( key, state ) keyStateCache[ "fastMove" ] = ( state == "down" ) end ) -- key_fastMove
bindKey( "lalt", "both", function( key, state ) keyStateCache[ "slowMove" ] = ( state == "down" ) end ) -- key_slowMove
bindKey( "w", "both", function( key, state ) keyStateCache[ "forward" ] = ( state == "down" ) end ) -- key_forward
bindKey( "s", "both", function( key, state ) keyStateCache[ "backward" ] = ( state == "down" ) end ) -- key_backward
bindKey( "a", "both", function( key, state ) keyStateCache[ "left" ] = ( state == "down" ) end ) -- key_left
bindKey( "d", "both", function( key, state ) keyStateCache[ "right" ] = ( state == "down" ) end ) -- key_right
...
local function freecamFrame ()
    ...
    if _getKeyState( "fastMove" ) then
        mspeed = options.fastMaxSpeed
	elseif _getKeyState( "slowMove" ) then
	...
	if options.smoothMovement then
	    ...
	    if ( _getKeyState( "forward" ) ) then
			speed = speed + acceleration 
	        speedKeyPressed = true
	    end
		if ( _getKeyState( "backward" ) ) then
			speed = speed - acceleration 
	        speedKeyPressed = true
	    end
		...
		if _getKeyState ( "key_left" ) then
			strafespeed = mspeed
		end
		if _getKeyState ( "key_right" ) then
			strafespeed = -mspeed
		end
	end
