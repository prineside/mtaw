<?php
	/*
		...
		Зависимости:
			- PHP 5, прописанный в PATH (для запуска через inspect.bat). Можно не прописывать в PATH и указывать полный путь к php.exe
			- Требует расширение mbstring, поэтому в файле php.ini необходимо его раскомментировать (убрать точку с запятой перед extension=php_mbstring.dll/so).
			  Файл php.ini находится в папке с интерпретатором PHP, если его нет - скопируйте php.ini-development рядом и переименуйте в php.ini, а затем раскоментируйте необходимые расширения (если окажется, что нужен не только mbstring) 
