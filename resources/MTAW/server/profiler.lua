...
--------------------------------------------------------------------------------
Profiler = {
	enabled = false;			-- Если false, модуль полностью отключен
	...
	excludeFiles = {			-- Исключенные из отладки файлы
		--[
		["@MTAW\\server\\debug.lua"] = true;
		["@MTAW\\server\\profiler.lua"] = true;
		["debug.lua"] = true;
		["profiler.lua"] = true;
		--]]
	};
	...
	init = function()
		addEventHandler( "Main.onServerLoad", resourceRoot, Profiler.onServerLoad )
	end;
	...
	onServerLoad = function()
		...
		if ( Profiler.enabled ) then
			...
			if ( Profiler.perFrame ) then
				setTimer( Profiler.onFrame, 50, 0 )
			end
		end
	end;
